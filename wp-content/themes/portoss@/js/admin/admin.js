...
jQuery( function( $ ) {
	...
	function portoAppendDialog( title, desc, btnYes = 'Yes', linkYes = '', btnNo = 'No', linkNo = '', extraClass = '', showAgain = false, cookieKey = false ) {
		...
		var $dialog = $( '.porto-admin-dialog' );
		if ( $dialog.length ) {
			$( '.porto-dialog-btn, .porto-dialog-overlay' ).off( 'click' );
			$dialog.remove();
		}
		...
		$( '.porto-dialog-btn, .porto-dialog-overlay' ).on( 'click', function( e ) {
			var $this = $( this ),
				href = $this.attr( 'href' ),
					...
					if ( response && response.content ) {
						if ( 'g' == importdata.type ) {
							...
							try {
								...
								console.warn( e );
								if ( e.message.indexOf( 'porto_cp_fbt' ) > -1 || e.message.indexOf( 'porto_cp_wishlist' ) > -1 || e.message.indexOf( 'porto_cp_compare' ) > -1 ) {
